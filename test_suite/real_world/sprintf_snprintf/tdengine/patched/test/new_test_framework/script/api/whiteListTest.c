...
void dropUsers(TAOS *taos) {
  ...
  for (int i = 0; i < nUser; ++i) {
    ...
    snprintf(qstr, sizeof(qstr), "DROP USER %s", users[i]);
    queryDB(taos, qstr);
    taos_close(taosu[i]);
 }
}
...
void createUsers(TAOS *taos, const char *host) {
  ...
  for (int i = 0; i < nUser; ++i) {
    sprintf(users[i], "user%d", i);
    snprintf(qstr, sizeof(qstr), "CREATE USER %s PASS 'taosdata'", users[i]);
    queryDB(taos, qstr);
    ...
    snprintf(qstr, sizeof(qstr), "alter user %s add host '%d.%d.%d.%d/24'", users[i], i, i, i, i);
    queryDB(taos, qstr);
