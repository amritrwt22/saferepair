...
int64_t getDirectorySize(char* dir) {
  ...
  while ((pDirEntry = taosReadDir(pDir)) != NULL) {
    ...
    snprintf(subdir, sizeof(subdir), "%s/%s", dir, fileName);
...
int32_t init_env() {
  ...
  snprintf(sqlStr, sizeof(sqlStr), "create database if not exists %s vgroups %d", g_stConfInfo.dbName, g_stConfInfo.numOfVgroups);
  ...
  snprintf(sqlStr, sizeof(sqlStr), "use %s", g_stConfInfo.dbName);
  ...
  snprintf(sqlStr, sizeof(sqlStr), "create topic test_stb_topic_1 as select ts,c0 from %s", g_stConfInfo.stbName);
