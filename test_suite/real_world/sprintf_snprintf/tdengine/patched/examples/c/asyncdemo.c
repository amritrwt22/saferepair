...
int main(int argc, char *argv[])
{
  ...
  snprintf(sql, sizeof(sql), "drop database if exists %s", db);
  queryDB(taos, sql);

  snprintf(sql, sizeof(sql), "create database %s", db);
  queryDB(taos, sql);
  ...
  for (i = 0; i < numOfTables; ++i) {
    ...
    snprintf(sql, sizeof(sql), "create table %s%d (ts timestamp, volume bigint)", prefix, i);
    queryDB(taos, sql);
  }  
  ...
  for (i = 0; i<numOfTables; ++i) {
    ...
    snprintf(sql, sizeof(sql), "insert into %s values(%ld, 0)", tableList[i].name, 1546300800000 + i);
  ...
  for (i = 0; i < numOfTables; ++i) {
    ...
    snprintf(sql, sizeof(sql), "select * from %s", tableList[i].name);
...
void taos_insert_call_back(void *param, TAOS_RES *tres, int code)
{
  ...
  if (pTable->rowsTried < points) {
    ...
    snprintf(sql, sizeof(sql), "insert into %s values(%ld, %d)", pTable->name, 1546300800000+pTable->rowsTried*1000, pTable->rowsTried);
