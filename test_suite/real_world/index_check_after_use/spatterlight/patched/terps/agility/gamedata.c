/*  gamedata.c-- Miscellaneous utilities and data structures */
...
static word search0_dict(const char *s)
{
  ...
  return hash[i];
#else
  for(i=0; i<dp && strcmp(s,dict[i])!=0; i++);
  if (i<dp) return i;
  return -1;
#endif
}
...
static void verb_multiword(int vp) 
{
  ...
  for(i=auxsyn[vp];syntbl[i]!=0;i++) {
    ...
    if (ptr!=0) {
      num_auxcomb+=1;
      auxcomb=rrealloc(auxcomb,num_auxcomb*sizeof(slist));
...
{ 
  ...
  for(i=0;i<DVERB;i++)
    {
      snprintf(buff,sizeof(buff),"dummy_verb%d",i+1);
  ...
  for(i=0;i<MAX_SUB;i++)
    {
      snprintf(buff,sizeof(buff),"subroutine%d",i+1);
  ...
  rfree(old_agt_verb);
  old_agt_verb=rmalloc(sizeof(word)*(i+1)); 
  for(i=0;old_agt_verb_str[i]!=NULL;i++) {
  ...
  if (aver<AGX00) {
    ...
    for(i=DVERB-1;i>=0;i--) {
      ...
      for(j=synlist[BASE_VERB+ii];syntbl[j]!=0;j++)
        if (w==syntbl[j]) return BASE_VERB+ii;
    }
...
char *objname(int i)  /* returns malloc'd name string of object i */
{
 ...
 /* At this point we can't get a name: return ILLn. */
  sLen = 3+1+(5*sizeof(int))/2+1;
  s=rmalloc(sLen);
         /* Make sure we have enough space in case i is big */
  snprintf(s,sLen,"ILL%d",i);
  return s;
}
...
static word check_comb(int combptr, int verb, int noun)
{
  word w;

  if (combptr==0) return 0;
  w=syntbl[combptr];
  if (syntbl[combptr+1]!=verb) return 0;
  if (syntbl[combptr+2]!=noun) return 0;
...
void cmds_syns_canon(void)
{
  ...
  for(i=0;i<last_cmd;i++) {
    /* VERB NOUN only */
    if(command[i].verbcmd>0 && command[i].nouncmd>0 &&
      command[i].prep==0 && command[i].objcmd==0) {
  ...
  */
{
  ...
  if (ss_size<ss_end+leng+1) {
    ...
    static_str=rrealloc(static_str,sizeof(char)*ss_size);
  }
...
long lookup_objflag(int id, int t, char *ofs)
{
  ...
  switch (t) 
    {
    case 0: *ofs=attrtable[id].rbit; return attrtable[id].r;
    case 1: *ofs=attrtable[id].nbit; return attrtable[id].n; 
    case 2: *ofs=attrtable[id].cbit; return attrtable[id].c; 
    default: rprintf("INT ERROR: Invalid object type.\n");
      return -1;
    }
}
...
long lookup_objprop(int id, int t)
{
  ...
  switch (t) 
    {
    case 0: return proptable[id].r;
    case 1: return proptable[id].n;
    case 2: return proptable[id].c;
    default: rprintf("INT ERROR: Invalid object type.\n");
      return -1;
    }
}
...
int num_oattrs(int t, rbool isflag)
{
  switch(t) 
    {
    case 0: return isflag ? num_rflags : num_rprops;
    case 1: return isflag ? num_nflags : num_nprops;
    case 2: return isflag ? num_cflags : num_cprops;
    default: rprintf("INT ERROR: Invalid object type.\n");
        return 0;
    }
}
...
rbool op_objflag(int op, integer obj, int id)
{
  ...
  if (troom(obj)) {t=0;firstobj=first_room;}
  else if (tnoun(obj)) {t=1;firstobj=first_noun;}
  else if (tcreat(obj)) {t=2;firstobj=first_creat;}
  else return 0;
