/*  parser.c-- the parser and the menu manager.           */
...
{
  rfree(lnoun);rfree(lobj);
  lnoun=lobj=NULL;
}


/* Print out parse error message and abort the parse */
static int parseerr(int msgid,char *s,int n)
{
  if (n>=0)
...
/* Output the parser's analysis of the current input line; used
   for debugging */
static int parse_out(parse_rec *lactor,int vb,parse_rec *lnoun,int prep,
	      parse_rec *lobj) 
{
  writeln("ANALYSIS:");
  writestr("Actor: ");print_nlist(lactor);
  writestr("Verb:");writeln(dict[ syntbl[auxsyn[vb]] ]);
  writestr("DObj: ");print_nlist(lnoun);
  writestr("Prep: ");
  if (prep!=0) writeln(dict[prep]); else writeln("---");
  writestr("IObj: ");print_nlist(lobj); 
  return 0;
}
...
static void save_input(void)
{
  ...
  for(i=0;i<MAXINPUT && input[i]!=-1;i++) {
...
static void restore_input(void)
{
  ...
  for(i=0;i<MAXINPUT && input_back[i]!=-1;i++) {
			    ...
			    word adj0, word noun0)
{
  ...
  pnew=rrealloc(pold,(n+2)*sizeof(parse_rec));
  pnew[n].obj=obj0;
  pnew[n].num=num0;
  pnew[n].info=info0;
  pnew[n].adj=adj0;
...
/* Remove record old[index] */
{
  ...
  pnew=rrealloc(old,i*sizeof(parse_rec)); /* Shrink it by one */
  return pnew;
}
...
{ 
  ...
  dest=rrealloc(dest,sizeof(parse_rec)*(i+j+1));
...
static parse_rec *convert_to_all(parse_rec *list, int *ofsref)
{
  ...
  list=rrealloc(list,(i+cnt)*sizeof(parse_rec));
...
static parse_rec *expand_all(parse_rec *lnoun)
{
  parse_rec *list;
  int i, j;
  ...
  objloop(i)
	...
	&& (lnoun==NULL || !scan_andrec(i,lnoun))) {
      temp_obj.obj=i;
  ...
  objloop(i)
	...
	if (it_scratch(j)) {kill_obj=1;break;}
			   ...
			   int ambig_type, rbool pick_one)
{
  ...
  case 0: 
    ...
    case 0:return (!PURE_ADJ || rec->info!=D_ADJ);
    case 1:return (rec->info!=D_INTERN);   /* Elim SCENE and DOOR */      
    case 2: return (rec->info!=D_NUM || rec->obj!=0); /* Eliminate numbers w/o 
  ...
  case 1:
	...
	max_disambig_score=rec->score;
      return 1;
    case 1: return (rec->score==max_disambig_score);
    case 2:
  ...
  case 2: 
    ...
    case 1:return (rec->info!=D_PRO && rec->info!=D_ALL &&  
		   rec->info!=D_INTERN && rec->info!=D_NUM);
    default: return 0;
    }
  case 3: return pick_one;   
  default: return 0;
  }
}
   ...
   nouns.  */
{
 ...
 /* Basic strategy: try to match nouns. If all matches are of length<=1,
  ...
  /* compare against the next word in the queue */
   while(num>0 && ip<MAXINPUT && input[ip]!=-1) {
     ip++;
     if (input[ip]==-1) break;
     num=0;
...
{
  parse_rec *next, *lnoun;
  int saveinfo;
  rbool all_except;

  all_except=0;
  next=lnoun=parse_a_noun();   
  saveinfo=next[0].info;

#if 0  /* Let the main parser sort this out */
  if (!and_ok) return lnoun; /* If no ANDs allowed, stop here. */
#endif
  ...
  if (is_actor) return lnoun; /* If no ANDs allowed, stop here. */

  if (lnoun[0].info==D_ALL && w_but(input[ip])) /* ALL EXCEPT ... */
 ...
 /* Now, itereate over <noun> AND <noun> AND ... */
  while ( (all_except || w_and(input[ip])) && 
	 saveinfo!=D_END)  /* && saveinfo!=D_ALL */
    {
      ...
      next=parse_a_noun(); saveinfo=next[0].info;
      if (next[0].info!=D_END) {            /* We found a word */
	if (!all_except) lnoun=add_rec(lnoun,AND_MARK,0,D_AND);
	lnoun=concat_list(lnoun,next); 
...
TELLHack:  /* This is used to restart the noun/prep/object scan
  ...
  if (prep==0) {  /* prep==0 unless we've met the special TURN ON|OFF case */
    pp=ip;prep=parse_prep();  /* Should be trivial */
    op=ip;lobj=parse_noun(prep==0,0); /* and_ok if no prep */
  }
  ...
  if (lactor[0].info==D_END && lnoun[0].info!=D_END && 
    ...
    {
      ip=op; /* Back up */
      rfree(lactor);rfree(lobj);
      lactor=lnoun;lnoun=NULL;
 ...
 /* Next we check to convert SHOOT <noun> AT <object> to 
  ...
  else if (lnoun[0].info==D_END && np!=pp && !w_isterm(input[np]))  
    /* i.e. invalid noun */
 ...
 /* Next, we need to determine if someone is trying to do 
  ...
  if (ambig_flag) {
    ...
    if (input[ip]==-1 && currnoun[0].info!=D_END) 
      {restore_input();ip=parse_ip;}   /* Yep, we're disambiguaing. */
...
/*Disambiguation routines; do it here instead of earlier to get
 ...
 provided it) */
  ...
  if (ambig_flag==-1) {ambig_flag=0;return 1;}
...
void menu_cmd(void)
{
  ...
  prep=verbinfo[choice].prep;
  objcnt=verbinfo[choice].objnum;

  /* Now identify the verb */
  ip=0;
  input[0]=verbword;input[1]=input[2]=-1;
  if (objcnt<=1 && prep!=0) input[1]=prep;
  vnum=id_verb();    

  lnoun=rmalloc(sizeof(parse_rec)*2);
  lnoun[0].obj=0;lnoun[0].num=0;lnoun[0].info=D_END;
  ...
  if (objcnt>=1) {
    ...
    nounmenu=rmalloc(nm_size*sizeof(menuentry));
    nm_width=0;
    for(i=0;i<nm_size;i++) {
    ...
    if (objcnt>=2 || prep==0)
      ...
      curr_cmd=concdup(dict[verbword],dict[prep]);

    choice=agt_menu(curr_cmd,nm_size,nm_width,nounmenu);
    rfree(curr_cmd);
    if (choice==-1 || doing_restore) 
      {rfree(nounmenu);rfree(nounval);rfree(lnoun);return;}

    if (objcnt==1 && prep!=0) { /* VERB PREP OBJ construction */ 
      ...
      lnoun[0].num=0;
      lnoun[0].info=D_NOUN;
      lnoun[1].obj=0;lnoun[1].num=0;lnoun[1].info=D_END;
    }
  }
  ...
  if (objcnt>=2) {
    ...
    rfree(tmp1);
    curr_cmd=concdup(tmp2,dict[prep]);  /* VERB NOUN PREP */
    rfree(tmp2);

    choice=agt_menu(curr_cmd,nm_size,nm_width,nounmenu);
    rfree(curr_cmd);
    if (choice==-1 || doing_restore) 
      {rfree(lnoun);rfree(nounmenu);rfree(nounval);return;}
  ...
  rfree(nounmenu);rfree(nounval);

  if (vnum==OLD_VERB+3) {  /* UNDO */
    v_undo();
    return;
  }
  ...
  tmpobj(&actrec);
  actrec.obj=0;
  exec(&actrec,vnum,lnoun,prep,&mobj);
