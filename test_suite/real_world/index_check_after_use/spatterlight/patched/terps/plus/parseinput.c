...
static char **SplitIntoWords(const char *string, int length)
{
    ...
    wordlength[0] = 0;
    int lastwasspace = 1;
    for (int i = 0; i < length && string[i] != 0 && word_index < MAX_WORDS; i++) {
        if (!isspace(string[i])) {
            if (lastwasspace) {
                /* Start a new word */
...
int IsNextParticiple(int partp, int noun2)
{
    ...
    if (TokenWords[WordIndex].Type == NOUN_TYPE && partp == 2 && (noun2 == 0 || TokenWords[WordIndex].Index == noun2)) {
        ...
        return 1;
    } else if (WordIndex < WordsInInput - 1 && TokenWords[WordIndex].Type == PREPOSITION_TYPE && (partp == 0 || TokenWords[WordIndex].Index == partp)) {
