...
static void CreateErrorMessage(const char *fchar, glui32 *second, const char *tchar)
{
    ...
    int i, j = 0, k = 0;
    for (i = 0; i < MAX_BUFFER && first[i] != 0; i++)
        buffer[i] = first[i];
    if (second != NULL) {
        for (j = 0; i + j < MAX_BUFFER && second[j] != 0; j++)
            buffer[i + j] = second[j];
    }
    if (third != NULL) {
        for (k = 0; i + j + k < MAX_BUFFER && third[k] != 0; k++)
...
void SplitIntoWords(glui32 *string, int length)
{
    ...
    wordlength[0] = 0;
    int lastwasspace = 1;
    for (int i = 0; i < length && string[i] != 0 && word_index < MAX_WORDS;
         i++) {
        foundspace = 0;
        switch (string[i]) {
        case 'y': {
