...
void heglk_adjust_guilty_bastards_windows(void)
{
    ...
    for (int i = 1; i < nwins; i ++)
    {
        if (wins[i].win && i != guilty_bastards_graphics_win && wins[i].win->type == wintype_Graphics)
        {
            gli_delete_window(wins[i].win);
            wins[i].win = NULL;
            wins[i].clear = 1;
...
//            LOG("hugo_settextwindow: found a match! (%d). main = %d, status = %d\n", i, mainwin, statuswin);
    ...
    // We assume that no status window is more than 20 chars high
...
//        LOG("Changing to a fixed-width font\n");
        ...
        if (inwindow && wins[curwin].l > 2 && wins[curwin].t > 2 && wins[curwin].win && wins[curwin].win->type == wintype_TextGrid)
        {
            glk_window_move_cursor(wins[curwin].win, currentpos, currentline - 1);
        }
    } else mono = false;
