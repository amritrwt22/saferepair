...
ex_diffgetput(exarg_T *eap)
{
	...
	while (p > eap->arg && VIM_ISWHITE(p[-1]))
	    --p;
	for (i = 0; eap->arg + i < p && vim_isdigit(eap->arg[i]); ++i)
	    ;
		...
		&& u_save(lnum - 1, lnum + count) != FAIL)
	{
		...
		// equal in them.
		for (i = 0; i < DB_COUNT; ++i)
		    if (i != idx_from && curtab->tp_diffbuf[i] != NULL
								&& i != idx_to
...
diff_fold_update(diff_T *dp, int skip_idx)
{
    ...
    FOR_ALL_WINDOWS(wp)
	for (i = 0; i < DB_COUNT; ++i)
	    if (i != skip_idx && curtab->tp_diffbuf[i] == wp->w_buffer)
