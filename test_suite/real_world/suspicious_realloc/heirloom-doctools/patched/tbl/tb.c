...
chspace(void) {
	...
	if (spcount>=maxvec) {
		n = maxvec + 20;
		{
		    void *_sr_tmp = realloc(spvecs, n * sizeof *spvecs);
		    if (_sr_tmp != NULL) {
		        spvecs = _sr_tmp;
		    }
		    /* else: realloc failed, spvecs still valid */
		}
		if (spvecs == NULL) {
...
alocv(int n) {
	...
	if (tpcount<0 || thisvec+n > tpvecs[tpcount]+MAXCHS) {
		...
		if (tpcount >= MAXPC) {
			{
			    void *_sr_tmp = realloc(tpvecs, (tpcount+1) * sizeof *tpvecs);
			    if (_sr_tmp != NULL) {
			        tpvecs = _sr_tmp;
			    }
			    /* else: realloc failed, tpvecs still valid */
			}
