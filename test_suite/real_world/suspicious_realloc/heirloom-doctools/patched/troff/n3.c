...
casesubstring(void)
{
	...
	while ((c = rbf()) != 0) {
		if (k >= sz) {
			sz += 512;
			{
			    void *_sr_tmp = realloc(tp, sz * sizeof *tp);
			    if (_sr_tmp != NULL) {
			        tp = _sr_tmp;
			    }
...
caseindex(void)
{
	...
	if (!skip(0)) {
		...
		while ((c = getch()) != 0 && !ismot(c) &&
			...
			if (ns >= as)
				{
				    void *_sr_tmp = realloc(sp, (as += 10) * sizeof *sp);
				    if (_sr_tmp != NULL) {
				        sp = _sr_tmp;
				    }
				    /* else: realloc failed, sp still valid */
				}
			sp[ns++] = i;
		}
...
caseunformat(int flag)
{
	...
	while ((c = rbf()) != 0) {
		if (ns >= as) {
			as += 512;
			{
			    void *_sr_tmp = realloc(tp, as * sizeof *tp);
			    if (_sr_tmp != NULL) {
			        tp = _sr_tmp;
			    }
			    /* else: realloc failed, tp still valid */
...
maybemore(int sofar, int flags)
{
	...
	if ((n = mapget(buf)) >= hadn) {
		...
		if (n >= alcd)
			{
			    void *_sr_tmp = realloc(had, (alcd += 20) * sizeof *had);
			    if (_sr_tmp != NULL) {
			        had = _sr_tmp;
			    }
			    /* else: realloc failed, had still valid */
			}
		had[n] = strdup(buf);
		hadn = n+1;
		mapadd(buf, n);
