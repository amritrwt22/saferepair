...
growfonts(int n)
{
	...
	{
	    void *_sr_tmp = realloc(fontbase, n * sizeof *fontbase);
	    if (_sr_tmp != NULL) {
	        fontbase = _sr_tmp;
	    }
	    /* else: realloc failed, fontbase still valid */
	}
	memset(&fontbase[Nfont], 0, (n - Nfont) * sizeof *fontbase);
	{
	    void *_sr_tmp = realloc(fontab, n * sizeof *fontab);
	    if (_sr_tmp != NULL) {
	        fontab = _sr_tmp;
	    }
	    /* else: realloc failed, fontab still valid */
	}
	memset(&fontab[Nfont], 0, (n - Nfont) * sizeof *fontab);
	{
	    void *_sr_tmp = realloc(kerntab, n * sizeof *kerntab);
	    if (_sr_tmp != NULL) {
	        kerntab = _sr_tmp;
	    }
	    /* else: realloc failed, kerntab still valid */
	}
	memset(&kerntab[Nfont], 0, (n - Nfont) * sizeof *kerntab);
	{
	    void *_sr_tmp = realloc(codetab, n * sizeof *codetab);
	    if (_sr_tmp != NULL) {
	        codetab = _sr_tmp;
	    }
	    /* else: realloc failed, codetab still valid */
	}
	memset(&codetab[Nfont], 0, (n - Nfont) * sizeof *codetab);
	{
	    void *_sr_tmp = realloc(fitab, n * sizeof *fitab);
	    if (_sr_tmp != NULL) {
	        fitab = _sr_tmp;
	    }
	    /* else: realloc failed, fitab still valid */
	}
	...
	{
	    void *_sr_tmp = realloc(cstab, n * sizeof *cstab);
	    if (_sr_tmp != NULL) {
	        cstab = _sr_tmp;
	    }
	    /* else: realloc failed, cstab still valid */
	}
	memset(&cstab[Nfont], 0, (n - Nfont) * sizeof *cstab);
	{
	    void *_sr_tmp = realloc(ccstab, n * sizeof *ccstab);
	    if (_sr_tmp != NULL) {
	        ccstab = _sr_tmp;
	    }
	    /* else: realloc failed, ccstab still valid */
	}
	memset(&ccstab[Nfont], 0, (n - Nfont) * sizeof *ccstab);
	bdtab = realloc(bdtab, n * sizeof *bdtab);
	memset(&bdtab[Nfont], 0, (n - Nfont) * sizeof *bdtab);
	{
	    void *_sr_tmp = realloc(tracktab, n * sizeof *tracktab);
	    if (_sr_tmp != NULL) {
	        tracktab = _sr_tmp;
	    }
	    /* else: realloc failed, tracktab still valid */
	}
	memset(&tracktab[Nfont], 0, (n - Nfont) * sizeof *tracktab);
	{
	    void *_sr_tmp = realloc(fallbacktab, n * sizeof *fallbacktab);
	    if (_sr_tmp != NULL) {
	        fallbacktab = _sr_tmp;
	    }
	    /* else: realloc failed, fallbacktab still valid */
	}
	memset(&fallbacktab[Nfont], 0, (n - Nfont) * sizeof *fallbacktab);
	{
	    void *_sr_tmp = realloc(zoomtab, n * sizeof *zoomtab);
	    if (_sr_tmp != NULL) {
	        zoomtab = _sr_tmp;
	    }
	    /* else: realloc failed, zoomtab still valid */
	}
