...
addunitab(int c __actual_use, int u __actual_use)
{
	...
	if (c >= a->nunitab) {
		{
		    void *_sr_tmp = realloc(a->unitab, (c+1) * sizeof *a->unitab);
		    if (_sr_tmp != NULL) {
		        a->unitab = _sr_tmp;
		    }
		    /* else: realloc failed, a->unitab still valid */
		}
		...
		if ((c2 = open_class(ClassDef2)) != NULL) {
			...
			while (get_class(c2, &g, &v), g >= 0) {
				...
				if (n >= a) {
					a = a ? 2*a : 128;
					{
					    void *_sr_tmp = realloc(g2, a * sizeof *g2);
					    if (_sr_tmp != NULL) {
					        g2 = _sr_tmp;
					    }
					    /* else: realloc failed, g2 still valid */
					}
					{
					    void *_sr_tmp = realloc(v2, a * sizeof *v2);
					    if (_sr_tmp != NULL) {
					        v2 = _sr_tmp;
					    }
					    /* else: realloc failed, v2 still valid */
					}
				}
				g2[n] = g;
				v2[n] = v;
...
add_feature(const char *name)
{
	...
	for (i = 0; a->features[i]; i++)
		if (strcmp(a->features[i]->name, name) == 0)
			return a->features[i];
	{
	    void *_sr_tmp = realloc(a->features, (i+2) * sizeof *a->features);
	    if (_sr_tmp != NULL) {
	        a->features = _sr_tmp;
	    }
	    /* else: realloc failed, a->features still valid */
	}
...
{
	{
	    void *_sr_tmp = realloc(fp->pairs, (fp->npairs+1) * sizeof *fp->pairs);
	    if (_sr_tmp != NULL) {
	        fp->pairs = _sr_tmp;
	    }
	    /* else: realloc failed, fp->pairs still valid */
	}
