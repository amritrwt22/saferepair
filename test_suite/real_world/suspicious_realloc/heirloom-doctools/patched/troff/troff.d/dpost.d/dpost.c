 ...
 */
	...
	case 'X':			/* copy through - from troff */
		...
		for (;;) {
		    ...
		    if (sz - n < 4096)
			{
			    void *_sr_tmp = realloc(buf, sz += 4096);
			    if (_sr_tmp != NULL) {
			        buf = _sr_tmp;
			    }
 ...
 */
	...
	free(contents);
	{
	    void *_sr_tmp = realloc(afmfonts, (afmcount+1) * sizeof *afmfonts);
	    if (_sr_tmp != NULL) {
	        afmfonts = _sr_tmp;
	    }
	    /* else: realloc failed, afmfonts still valid */
	}
	afmfonts[afmcount] = a;
 ...
 */
	...
	while ( sget(temp, sizeof temp, fp) == 1 ) {
		...
		if ( sget(&temp[3], sizeof temp - 3, fp) == 1 &&
		    ...
		    if ( ++i % 10 == 0 )
			{
			    void *_sr_tmp = realloc(devfontmap, (i + 10) * sizeof(Devfontmap));
			    if (_sr_tmp != NULL) {
			        devfontmap = (Devfontmap *)_sr_tmp;
			    }
			    /* else: realloc failed, devfontmap still valid */
			}
		}   /* End if */
	    while ( (c = getc(fp)) != '\n' && c != EOF ) ;
	}   /* End while */
...
mbs2pdf(char *mp)
{
	...
	*ustr = '(';
	...
	while (*mp) {
	    ...
	    if (i + 4 >= sz)
		{
		    void *_sr_tmp = realloc(ustr, sz += 16);
		    if (_sr_tmp != NULL) {
		        ustr = _sr_tmp;
		    }
...
t_pdfmark(char *buf)
{
    ...
    *bp++ = '\0';
	...
	{
	    void *_sr_tmp = realloc(Bookmarks, ++nBookmarks*sizeof *Bookmarks);
	    if (_sr_tmp != NULL) {
	        Bookmarks = _sr_tmp;
	    }
	    /* else: realloc failed, Bookmarks still valid */
	}
