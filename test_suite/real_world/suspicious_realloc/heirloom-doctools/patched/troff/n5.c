...
casehcode(void)
{
	...
	do {
		...
		if (c && _d && !ismot(c) && !ismot(_d)) {
			if ((k = cbits(c)) >= nhcode) {
				{
				    void *_sr_tmp = realloc(hcode, (k+1) * sizeof *hcode);
				    if (_sr_tmp != NULL) {
				        hcode = _sr_tmp;
				    }
				    /* else: realloc failed, hcode still valid */
				}
	...
	if (skip(0)) {
		...
		for (n = 0; ; n++) {
			if (n+1 >= nlpfx) {
				nlpfx += 10;
				{
				    void *_sr_tmp = realloc(lpfx, nlpfx * sizeof *lpfx);
				    if (_sr_tmp != NULL) {
				        lpfx = _sr_tmp;
				    }
				    /* else: realloc failed, lpfx still valid */
				}
			}
			c = getch();
			if (nlflg)
...
open1(int flags)
{
	...
	if (skip(1) || !getname() || skip(1))
		return;
	{
	    void *_sr_tmp = realloc(streams, sizeof *streams * ++nstreams);
	    if (_sr_tmp != NULL) {
	        streams = _sr_tmp;
	    }
	    /* else: realloc failed, streams still valid */
	}
	...
	*nxevp = 0;
	...
	if (xflag == 0 || isdigit(c) || c == '(') {
		...
		do {
			...
			if (i >= sz)
				{
				    void *_sr_tmp = realloc(name, (sz += 8) * sizeof *name);
				    if (_sr_tmp != NULL) {
				        name = _sr_tmp;
				    }
				    /* else: realloc failed, name still valid */
				}
			name[i++] = c;
		} while (c);
		if (*name == 0) {
