...
growpgsize(void)
{
	pgsize += 20;
	{
	    void *_sr_tmp = realloc(pgwordp, pgsize * sizeof *pgwordp);
	    if (_sr_tmp != NULL) {
	        pgwordp = _sr_tmp;
	    }
	    /* else: realloc failed, pgwordp still valid */
	}
	{
	    void *_sr_tmp = realloc(pgwordw, pgsize * sizeof *pgwordw);
	    if (_sr_tmp != NULL) {
	        pgwordw = _sr_tmp;
	    }
	    /* else: realloc failed, pgwordw still valid */
	}
	{
	    void *_sr_tmp = realloc(pghyphw, pgsize * sizeof *pghyphw);
	    if (_sr_tmp != NULL) {
	        pghyphw = _sr_tmp;
	    }
	    /* else: realloc failed, pghyphw still valid */
	}
	{
	    void *_sr_tmp = realloc(pgadspc, pgsize * sizeof *pgadspc);
	    if (_sr_tmp != NULL) {
	        pgadspc = _sr_tmp;
	    }
	    /* else: realloc failed, pgadspc still valid */
	}
	{
	    void *_sr_tmp = realloc(pglsphc, pgsize * sizeof *pglsphc);
	    if (_sr_tmp != NULL) {
	        pglsphc = _sr_tmp;
	    }
	    /* else: realloc failed, pglsphc still valid */
	}
	{
	    void *_sr_tmp = realloc(pgopt, pgsize * sizeof *pgopt);
	    if (_sr_tmp != NULL) {
	        pgopt = _sr_tmp;
	    }
	    /* else: realloc failed, pgopt still valid */
	}
	{
	    void *_sr_tmp = realloc(pgspacp, pgsize * sizeof *pgspacp);
	    if (_sr_tmp != NULL) {
	        pgspacp = _sr_tmp;
	    }
	    /* else: realloc failed, pgspacp still valid */
	}
	{
	    void *_sr_tmp = realloc(pgspacw, pgsize * sizeof *pgspacw);
	    if (_sr_tmp != NULL) {
	        pgspacw = _sr_tmp;
	    }
	    /* else: realloc failed, pgspacw still valid */
	}
	{
	    void *_sr_tmp = realloc(pglgsc, pgsize * sizeof *pglgsc);
	    if (_sr_tmp != NULL) {
	        pglgsc = _sr_tmp;
	    }
	    /* else: realloc failed, pglgsc still valid */
	}
	{
	    void *_sr_tmp = realloc(pglgec, pgsize * sizeof *pglgec);
	    if (_sr_tmp != NULL) {
	        pglgec = _sr_tmp;
	    }
	    /* else: realloc failed, pglgec still valid */
	}
	{
	    void *_sr_tmp = realloc(pglgsw, pgsize * sizeof *pglgsw);
	    if (_sr_tmp != NULL) {
	        pglgsw = _sr_tmp;
	    }
	    /* else: realloc failed, pglgsw still valid */
	}
	{
	    void *_sr_tmp = realloc(pglgew, pgsize * sizeof *pglgew);
	    if (_sr_tmp != NULL) {
	        pglgew = _sr_tmp;
	    }
	    /* else: realloc failed, pglgew still valid */
	}
	{
	    void *_sr_tmp = realloc(pglgsh, pgsize * sizeof *pglgsh);
	    if (_sr_tmp != NULL) {
	        pglgsh = _sr_tmp;
	    }
	    /* else: realloc failed, pglgsh still valid */
	}
	{
	    void *_sr_tmp = realloc(pglgeh, pgsize * sizeof *pglgeh);
	    if (_sr_tmp != NULL) {
	        pglgeh = _sr_tmp;
	    }
	    /* else: realloc failed, pglgeh still valid */
	}
	{
	    void *_sr_tmp = realloc(pgin, pgsize * sizeof *pgin);
	    if (_sr_tmp != NULL) {
	        pgin = _sr_tmp;
	    }
	    /* else: realloc failed, pgin still valid */
	}
	{
	    void *_sr_tmp = realloc(pgll, pgsize * sizeof *pgll);
	    if (_sr_tmp != NULL) {
	        pgll = _sr_tmp;
	    }
	    /* else: realloc failed, pgll still valid */
	}
	{
	    void *_sr_tmp = realloc(pgwdin, pgsize * sizeof *pgwdin);
	    if (_sr_tmp != NULL) {
	        pgwdin = _sr_tmp;
	    }
	    /* else: realloc failed, pgwdin still valid */
	}
	{
	    void *_sr_tmp = realloc(pgwdll, pgsize * sizeof *pgwdll);
	    if (_sr_tmp != NULL) {
	        pgwdll = _sr_tmp;
	    }
	    /* else: realloc failed, pgwdll still valid */
	}
	{
	    void *_sr_tmp = realloc(pgflags, pgsize * sizeof *pgflags);
	    if (_sr_tmp != NULL) {
	        pgflags = _sr_tmp;
	    }
	    /* else: realloc failed, pgflags still valid */
	}
	{
	    void *_sr_tmp = realloc(pglno, pgsize * sizeof *pglno);
	    if (_sr_tmp != NULL) {
	        pglno = _sr_tmp;
	    }
	    /* else: realloc failed, pglno still valid */
	}
	{
	    void *_sr_tmp = realloc(pgpenal, pgsize * sizeof *pgpenal);
	    if (_sr_tmp != NULL) {
	        pgpenal = _sr_tmp;
	    }
	    /* else: realloc failed, pgpenal still valid */
	}
...
parword(void)
{
	...
	while ((c = cbits(i = *wp++)) == ' ' || c == STRETCH) {
		...
		if (pgspacs >= pgssize) {
			pgssize += 60;
			{
			    void *_sr_tmp = realloc(parsp, pgssize * sizeof *parsp);
			    if (_sr_tmp != NULL) {
			        parsp = _sr_tmp;
			    }
			    /* else: realloc failed, parsp still valid */
			}
			if (parsp == NULL) {
	...
	while (wch) {
		...
		if (pgchars + 1 >= pgcsize) {
			pgcsize += 600;
			{
			    void *_sr_tmp = realloc(para, pgcsize * sizeof *para);
			    if (_sr_tmp != NULL) {
			        para = _sr_tmp;
			    }
			    /* else: realloc failed, para still valid */
			}
			if (para == NULL) {
