...
pushinlev(void)
{
	if (ninlev >= ainlev) {
		ainlev += 4;
		{
		    void *_sr_tmp = realloc(inlevp, ainlev * sizeof *inlevp);
		    if (_sr_tmp != NULL) {
		        inlevp = _sr_tmp;
		    }
		    /* else: realloc failed, inlevp still valid */
		}
	}
...
casepsbb(void)
{
	...
	do {
		...
		if (n >= sz)
			{
			    void *_sr_tmp = realloc(buf, (sz += 14) * sizeof *buf);
			    if (_sr_tmp != NULL) {
			        buf = _sr_tmp;
			    }
			    /* else: realloc failed, buf still valid */
			}
		buf[n++] = c;
	} while (c);
	getpsbb(buf, bb);
...
morechars(int n)
{
	...
	{
	    void *_sr_tmp = realloc(widcache, nnc * sizeof *widcache);
	    if (_sr_tmp != NULL) {
	        widcache = _sr_tmp;
	    }
	    /* else: realloc failed, widcache still valid */
	}
	memset(&widcache[NCHARS], 0, (nnc-NCHARS) * sizeof *widcache);
	{
	    void *_sr_tmp = realloc(trtab, nnc * sizeof *trtab);
	    if (_sr_tmp != NULL) {
	        trtab = _sr_tmp;
	    }
	    /* else: realloc failed, trtab still valid */
	}
	{
	    void *_sr_tmp = realloc(trnttab, nnc * sizeof *trnttab);
	    if (_sr_tmp != NULL) {
	        trnttab = _sr_tmp;
	    }
	    /* else: realloc failed, trnttab still valid */
	}
	for (i = NCHARS; i < nnc; i++)
		trnttab[i] = trtab[i] = i;
	{
	    void *_sr_tmp = realloc(trintab, nnc * sizeof *trintab);
	    if (_sr_tmp != NULL) {
	        trintab = _sr_tmp;
	    }
	    /* else: realloc failed, trintab still valid */
	}
	memset(&trintab[NCHARS], 0, (nnc-NCHARS) * sizeof *trintab);
	{
	    void *_sr_tmp = realloc(gchtab, nnc * sizeof *gchtab);
	    if (_sr_tmp != NULL) {
	        gchtab = _sr_tmp;
	    }
	    /* else: realloc failed, gchtab still valid */
	}
	memset(&gchtab[NCHARS], 0, (nnc-NCHARS) * sizeof *gchtab);
	{
	    void *_sr_tmp = realloc(chartab, nnc * sizeof *chartab);
	    if (_sr_tmp != NULL) {
	        chartab = _sr_tmp;
	    }
	    /* else: realloc failed, chartab still valid */
	}
	memset(&chartab[NCHARS], 0, (nnc-NCHARS) * sizeof *chartab);
#ifndef	NROFF
	{
	    void *_sr_tmp = realloc(fchartab, nnc * sizeof *fchartab);
	    if (_sr_tmp != NULL) {
	        fchartab = _sr_tmp;
	    }
	    /* else: realloc failed, fchartab still valid */
	}
