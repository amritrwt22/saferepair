...
getname(void)
{
	...
	while (1) {
		...
		if (k + 1 >= NS)
			{
			    void *_sr_tmp = realloc(nextf, NS += 14);
			    if (_sr_tmp != NULL) {
			        nextf = _sr_tmp;
			    }
			    /* else: realloc failed, nextf still valid */
			}
		nextf[k++] = j & BYTEMASK;
	}
	nextf[k] = 0;
...
_setenv(void)
{
	...
	case '[':
		for (;;) {
			if (i + 1 >= a)
				{
				    void *_sr_tmp = realloc(np, a += 32);
				    if (_sr_tmp != NULL) {
				        np = _sr_tmp;
				    }
				    /* else: realloc failed, np still valid */
				}
			if ((c = getach()) == 0) {
				nodelim(']');
				break;
	...
	if (skip(0))
		...
		while (c = getch(), !nlflg) {
			if (i + 3 >= size) {
				size += 10;
				{
				    void *_sr_tmp = realloc(tp, size * sizeof *tp);
				    if (_sr_tmp != NULL) {
				        tp = _sr_tmp;
				    }
				    /* else: realloc failed, tp still valid */
				}
			}
			tp[i++] = c;
		}
...
prepchar(struct fmtchar *fp)
{
	...
	if (charcount >= charoutsz) {
		charoutsz += 32;
		{
		    void *_sr_tmp = realloc(charout, charoutsz * sizeof *charout);
		    if (_sr_tmp != NULL) {
		        charout = _sr_tmp;
		    }
		    /* else: realloc failed, charout still valid */
		}
	}
