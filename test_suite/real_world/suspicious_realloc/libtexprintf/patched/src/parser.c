...
/* make regular brackets (a) |a| [a] ‖a‖ */
{
	...
	if (h<1)
		h=1;
	{
	    void *_sr_tmp = realloc(posbox->content, (2*(h+posbox->Nc))*sizeof(int));
	    if (_sr_tmp != NULL) {
	        posbox->content = _sr_tmp;
	    }
	    /* else: realloc failed, posbox->content still valid */
	}
...
void AddBoxBelowAbove(box *b, unsigned int u, boxalign A, int above, int rep, int Font)
{
	...
	if (rep)
	{
		...
		{
		    void *_sr_tmp = realloc(ustr, (n*w+1)*sizeof(char));
		    if (_sr_tmp != NULL) {
		        ustr = _sr_tmp;
		    }
		    /* else: realloc failed, ustr still valid */
		}
		for (i=1;i<w;i++)
		{
			for (j=0;j<n;j++)
		...
		if ((style->avoidcombining==0)||(alt==0))
		{
			if (UnitBoxCount(stuff)==1)
			{
				...
				if (strspaces(str)==1)
				{
					...
					{
					    void *_sr_tmp = realloc(str, (l+6)*sizeof(char));
					    if (_sr_tmp != NULL) {
					        str = _sr_tmp;
					    }
					    /* else: realloc failed, str still valid */
					}
					strncpy(str+l,cm,5);
...
void MakeSqrt(TOKEN *T, box *b, int Font)
{
	...
	{
	    void *_sr_tmp = realloc(sqrt->content, (2*(h+(h/2+1)+3+w)+sqrt->Nc)*sizeof(int));
	    if (_sr_tmp != NULL) {
	        sqrt->content = _sr_tmp;
	    }
	    /* else: realloc failed, sqrt->content still valid */
	}
	xy=(int *)sqrt->content;
	if (ioff)
	{
...
void DrawInt(box *b, int n, int size)
{
	...
	if (size<=1)
	{
		switch(n)
		{
			...
			case 4:
				{
				    void *_sr_tmp = realloc(integ->content, (8*sizeof(int)));
				    if (_sr_tmp != NULL) {
				        integ->content = _sr_tmp;
				    }
				    /* else: realloc failed, integ->content still valid */
				}
			...
			case 5:
				{
				    void *_sr_tmp = realloc(integ->content, (6*sizeof(int)));
				    if (_sr_tmp != NULL) {
				        integ->content = _sr_tmp;
				    }
				    /* else: realloc failed, integ->content still valid */
	...
	else
	{
		...
		{
			{
			    void *_sr_tmp = realloc(integ->content, (2*(2*size+integ->Nc+1)*sizeof(int)));
			    if (_sr_tmp != NULL) {
			        integ->content = _sr_tmp;
			    }
			    /* else: realloc failed, integ->content still valid */
		...
		{
			{
			    void *_sr_tmp = realloc(integ->content, (2*(n*size+integ->Nc)*sizeof(int)));
			    if (_sr_tmp != NULL) {
			        integ->content = _sr_tmp;
			    }
			    /* else: realloc failed, integ->content still valid */
...
void DrawOInt(box *b, int n, int size)
{
	...
	else
	{
		...
		{
			{
			    void *_sr_tmp = realloc(integ->content, (2*(2*size+integ->Nc+3)*sizeof(int)));
			    if (_sr_tmp != NULL) {
			        integ->content = _sr_tmp;
			    }
			    /* else: realloc failed, integ->content still valid */
		...
		{
			{
			    void *_sr_tmp = realloc(integ->content, (2*(n*size+integ->Nc+2)*sizeof(int)));
			    if (_sr_tmp != NULL) {
			        integ->content = _sr_tmp;
			    }
			    /* else: realloc failed, integ->content still valid */
...
void RescaleVsep(box *vsep, int h)
{
	...
	xy=(int *)vsep->content;
	{
	    void *_sr_tmp = realloc(xy, 2*h*sizeof(int));
	    if (_sr_tmp != NULL) {
	        xy = _sr_tmp;
	    }
	    /* else: realloc failed, xy still valid */
	}
	vsep->content=xy;
	for (i=0;i<h;i++)
	{
...
void RescaleHsep(box *hsep, int w)
{
	...
	xy=(int *)hsep->content;
	{
	    void *_sr_tmp = realloc(xy, 2*w*sizeof(int));
	    if (_sr_tmp != NULL) {
	        xy = _sr_tmp;
	    }
	    /* else: realloc failed, xy still valid */
