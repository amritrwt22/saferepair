...
void AddChild(box * parent, boxtype T, void *content)
{
	if (!parent->child)
		...
		{
		    void *_sr_tmp = realloc(parent->child, (parent->Nc+1)*sizeof(box));
		    if (_sr_tmp != NULL) {
		        parent->child = _sr_tmp;
		    }
		    /* else: realloc failed, parent->child still valid */
		}
	...
	for (i=0;i<b->Nc;i++)
	{
		...
		if (((LW>0)&&(X+b->child[i].w>LW)&&(X>0))||(b->child[i].T==B_ENDLINE))/* if X = 0 -> w>LW, we stay on this line */
		{
			...
			if (nl==nra)
			{
				nra+=2;
				{
				    void *_sr_tmp = realloc(y, nra*sizeof(int));
				    if (_sr_tmp != NULL) {
				        y = _sr_tmp;
				    }
				...
				{
				    void *_sr_tmp = realloc(yc, nra*sizeof(int));
				    if (_sr_tmp != NULL) {
				        yc = _sr_tmp;
				    }
				    /* else: realloc failed, yc still valid */
				}
			}
		}
		b->child[i].rx=X;
