...
char * Symbols_Str()
{
	...
	while(Symbols[j].name)
	{
		...
		while (s < (ln+slen+1))
			s*=2;
		{
		    void *_sr_tmp = realloc(line, s);
		    if (_sr_tmp != NULL) {
		        line = _sr_tmp;
		    }
		    /* else: realloc failed, line still valid */
		}
		...
		case PD_OVER: /* same as \frac but then after the fact. BUG: in tex this command is greedy but this implementation is not! */
		{
			...
			else
			{
				...
				for (i=0;i<T->Nargs;i++)
					free(T->args[i]);					
				{
				    void *_sr_tmp = realloc(T->args, 2*sizeof(char *));
				    if (_sr_tmp != NULL) {
				        T->args = _sr_tmp;
				    }
				    /* else: realloc failed, T->args still valid */
				}
			}
			T->Nargs=2;
					...
					switch (*begin)
					{
						case '&': /* next column */
							...
							if (i==Na)
							{
								Na+=10;
								{
								    void *_sr_tmp = realloc(res, Na*sizeof(char*));
								    if (_sr_tmp != NULL) {
								        res = _sr_tmp;
								    }
								    /* else: realloc failed, res still valid */
								}
							}
							na=10;
	...
	while (string[p])
	{
		if (strncmp(string+p, op,nop)==0)
		{
			...
			if (string[q]!='}')
			{
				...
				na+=2;
				{
				    void *_sr_tmp = realloc(string, na*sizeof(char));
				    if (_sr_tmp != NULL) {
				        string = _sr_tmp;
				    }
				    /* else: realloc failed, string still valid */
				}
				q=end;
				end+=2;
			...
			if (S.name!=NULL)
			{
				...
				if(IsCombiningMark(S.unicode))
				{
						...
						if (Ncomb==Nacomb)
						{
							Nacomb+=4;
							{
							    void *_sr_tmp = realloc(comb, Nacomb*sizeof(int));
							    if (_sr_tmp != NULL) {
							        comb = _sr_tmp;
							    }
							    /* else: realloc failed, comb still valid */
							}
						}
						free(str);
				}
