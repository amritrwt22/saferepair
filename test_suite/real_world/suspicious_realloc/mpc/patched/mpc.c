...
static void mpc_input_mark(mpc_input_t *i) {
  ...
  if (i->marks_num > i->marks_slots) {
    ...
    {
        void *_sr_tmp = realloc(i->lasts, sizeof(char) * i->marks_slots);
        if (_sr_tmp != NULL) {
            i->lasts = _sr_tmp;
        }
        /* else: realloc failed, i->lasts still valid */
    }
  }
...
static int mpc_input_success(mpc_input_t *i, char c, char **o) {
  ...
  &&  i->buffer && !mpc_input_buffer_in_range(i)) {
    {
        void *_sr_tmp = realloc(i->buffer, strlen(i->buffer) + 2);
        if (_sr_tmp != NULL) {
            i->buffer = _sr_tmp;
        }
        /* else: realloc failed, i->buffer still valid */
    }
...
mpc_parser_t *mpc_new(const char *name) {
  ...
  p->retained = 1;
  {
      void *_sr_tmp = realloc(p->name, strlen(name) + 1);
      if (_sr_tmp != NULL) {
          p->name = _sr_tmp;
      }
      /* else: realloc failed, p->name still valid */
  }
  strcpy(p->name, name);
  return p;
}
...
mpc_parser_t *mpc_failf(const char *fmt, ...) {
  ...
  {
      void *_sr_tmp = realloc(buffer, strlen(buffer) + 1);
      if (_sr_tmp != NULL) {
          buffer = _sr_tmp;
      }
      /* else: realloc failed, buffer still valid */
  }
...
static mpc_val_t *mpcf_re_range(mpc_val_t *x) {
  ...
  for (i = comp; i < strlen(s); i++){
    ...
    if (s[i] == '\\') {
      ...
      if (tmp != NULL) {
        {
            void *_sr_tmp = realloc(range, strlen(range) + strlen(tmp) + 1);
            if (_sr_tmp != NULL) {
                range = _sr_tmp;
            }
            /* else: realloc failed, range still valid */
        }
        strcat(range, tmp);
      } else {
        {
            void *_sr_tmp = realloc(range, strlen(range) + 1 + 1);
            if (_sr_tmp != NULL) {
                range = _sr_tmp;
            }
            /* else: realloc failed, range still valid */
    ...
    /* Regex Range...Range */
      ...
      if (s[i+1] == '\0' || i == 0) {
          {
              void *_sr_tmp = realloc(range, strlen(range) + strlen("-") + 1);
              if (_sr_tmp != NULL) {
                  range = _sr_tmp;
              }
              /* else: realloc failed, range still valid */
        ...
        for (j = start; j <= end; j++) {
          {
              void *_sr_tmp = realloc(range, strlen(range) + 1 + 1 + 1);
              if (_sr_tmp != NULL) {
                  range = _sr_tmp;
              }
              /* else: realloc failed, range still valid */
...
static mpc_val_t *mpcf_escape_new(mpc_val_t *x, const char *input, const char **output) {
  ...
  while (*s) {
    ...
    while (output[i]) {
      if (*s == input[i]) {
        {
            void *_sr_tmp = realloc(y, strlen(y) + strlen(output[i]) + 1);
            if (_sr_tmp != NULL) {
                y = _sr_tmp;
            }
            /* else: realloc failed, y still valid */
        }
        strcat(y, output[i]);
        found = 1;
        break;
    ...
    if (!found) {
      {
          void *_sr_tmp = realloc(y, strlen(y) + 2);
          if (_sr_tmp != NULL) {
              y = _sr_tmp;
          }
          /* else: realloc failed, y still valid */
  ...
  while (*s) {
    ...
    while (output[i]) {
      if ((*(s+0)) == output[i][0] &&
          (*(s+1)) == output[i][1]) {
        {
            void *_sr_tmp = realloc(y, strlen(y) + 1 + 1);
            if (_sr_tmp != NULL) {
                y = _sr_tmp;
            }
            /* else: realloc failed, y still valid */
...
mpc_ast_t *mpc_ast_add_child(mpc_ast_t *r, mpc_ast_t *a) {
  r->children_num++;
  {
      void *_sr_tmp = realloc(r->children, sizeof(mpc_ast_t*) * r->children_num);
      if (_sr_tmp != NULL) {
          r->children = _sr_tmp;
      }
      /* else: realloc failed, r->children still valid */
  }
...
mpc_ast_t *mpc_ast_add_tag(mpc_ast_t *a, const char *t) {
  if (a == NULL) { return a; }
  {
      void *_sr_tmp = realloc(a->tag, strlen(t) + 1 + strlen(a->tag) + 1);
      if (_sr_tmp != NULL) {
          a->tag = _sr_tmp;
      }
      /* else: realloc failed, a->tag still valid */
  }
...
mpc_ast_t *mpc_ast_tag(mpc_ast_t *a, const char *t) {
  {
      void *_sr_tmp = realloc(a->tag, strlen(t) + 1);
      if (_sr_tmp != NULL) {
          a->tag = _sr_tmp;
      }
      /* else: realloc failed, a->tag still valid */
...
static mpc_parser_t *mpca_grammar_find_parser(char *x, mpca_grammar_st_t *st) {
  ...
  if (is_number(x)) {
    ...
    while (st->parsers_num <= i) {
      st->parsers_num++;
      {
          void *_sr_tmp = realloc(st->parsers, sizeof(mpc_parser_t*) * st->parsers_num);
          if (_sr_tmp != NULL) {
              st->parsers = _sr_tmp;
          }
          /* else: realloc failed, st->parsers still valid */
      }
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
  ...
  while (1) {
    ...
    && !p->data.or.xs[p->data.or.n-1]->retained) {
      ...
      p->data.or.n = n + m - 1;
      {
          void *_sr_tmp = realloc(p->data.or.xs, sizeof(mpc_parser_t*) * (n + m -1));
          if (_sr_tmp != NULL) {
              p->data.or.xs = _sr_tmp;
          }
          /* else: realloc failed, p->data.or.xs still valid */
      }
    ...
    &&  p->data.and.xs[0]->data.and.f == mpcf_fold_ast) {
      ...
      p->data.and.n = n + m - 1;
      {
          void *_sr_tmp = realloc(p->data.and.xs, sizeof(mpc_parser_t*) * (n + m - 1));
          if (_sr_tmp != NULL) {
              p->data.and.xs = _sr_tmp;
          }
          /* else: realloc failed, p->data.and.xs still valid */
      }
      {
          void *_sr_tmp = realloc(p->data.and.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
          if (_sr_tmp != NULL) {
              p->data.and.dxs = _sr_tmp;
          }
          /* else: realloc failed, p->data.and.dxs still valid */
      }
